...
MASA = 0.04593 # 45.93 g
radioReal = RADIO_M * PX_M * 30 
...
def run():
	...
	# Definimos el suelo
    jlib.crearSuelo(space, (0, NIVEL_DEL_SUELO), (HOYO_M * PX_M - RADIO_M * PX_M * 220, NIVEL_DEL_SUELO), grosor=0, friction=1.8, elasticity=0.05)

    jlib.crearSuelo(space, (HOYO_M * PX_M + RADIO_M * PX_M * 220, NIVEL_DEL_SUELO), (WIDTH, NIVEL_DEL_SUELO), grosor=0, friction=1.8, elasticity=0.05)
    

	# Definimos la bola
    body, shape = jlib.crearCirculo(space, masa=MASA, radio=RADIO_M*30, friction=0.7, elasticity=0.91, posX=50, posY=NIVEL_DEL_SUELO - radioReal - 1)
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    ...
                    v_bp = ((1 + e) * m_c / (MASA + m_c)) * v_cx * math.cos(loft_rad)
        ...
        pygame.draw.circle(screen, (255, 255, 255), (pos_x, pos_y), int(radioReal))
        
        # Dibujar el Hoyo (negro en el suelo para diferenciarlo)
        pygame.draw.circle(screen, (0, 0, 0), (int(HOYO_M * PX_M), int(NIVEL_DEL_SUELO)), int(radioReal))
