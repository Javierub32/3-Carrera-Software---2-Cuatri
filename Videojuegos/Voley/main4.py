...
class Tsuelo(Tobjeto):
    def __init__(self, sim):
        ...
        # Hacemos el suelo el doble de largo internamente para que la pelota no se caiga al vacío si llega muy lejos
...
class TVoley(Tsim):
    def __init__(self, **kwargs):
        ...
        self.pos_inicio_balon = (1.5, 3.1) 
        self.lanzado = False 
        self.magnus_activo = True
        
        self.ha_tocado_suelo = False
        self.distancia_impacto = None
        self.velocidad_y_anterior = 0 # Variable para guardar la velocidad vertical
        
        self.v_lanzamiento = 20.0    
    ...
    def draw(self):
        ...
        txt_mag = self.font.render(f"Efecto Magnus: {'SÍ' if self.magnus_activo else 'NO'}", True, color_mag)
    ...
    def revisar_impacto(self):
        if self.lanzado and not self.ha_tocado_suelo:
            vel_y_actual = self.balon.velocidad.y
            
            # --- NUEVA LÓGICA ---
            # Si la pelota iba cayendo (velocidad < 0) y ahora sube tras rebotar (velocidad >= 0)
            # Y se encuentra cerca del suelo (a menos de 0.5 metros de altura)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Cierre silencioso si se interrumpe desde consola
        pass
    finally:
        pygame.quit()