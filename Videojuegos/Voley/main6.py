...
class Tsuelo(Tobjeto):
    def __init__(self, sim):
        ...
        self.p2 = Vec2d(self.sim.width * 2, self.sim.suelo)
    ...
    def draw(self):
        ...
        pygame.draw.line(self.sim.screen, (255, 255, 255), self.p1, Vec2d(self.sim.width, self.sim.suelo), 5)
        for x_m in [0, 9, 18]:
...
class TbalonVoley(Tobjeto):
    def __init__(self, sim, pos_m):
        ...
        moment = 2/3*self.masa*(self.radio_px**2)
    ...
    def aplicar_aerodinamica(self, body, gravity, damping, dt):
        # La gravedad no hace efecto si está en la mano del jugador (estado 0)
        if hasattr(self.sim, 'estado_saque') and self.sim.estado_saque == 0:
            body.velocity = (0, 0)
    ...
    def _preparar_imagen(self):
        factor_visual = 1.5
        radio_visual = int(self.radio_px * factor_visual)
        diametro = int(radio_visual * 2)
        
        surf = pygame.Surface((diametro, diametro), pygame.SRCALPHA)
        pygame.draw.circle(surf, (255, 220, 50), (radio_visual, radio_visual), radio_visual)
        grosor = max(2, int(6 * factor_visual / 2))
        pygame.draw.arc(surf, (0, 50, 200), (0, 0, diametro, diametro), 0, math.pi, grosor)
        pygame.draw.arc(surf, (0, 50, 200), (0, 0, diametro, diametro), math.pi/2, 3*math.pi/2, grosor)
        return surf
...
class TjugadorVoley(Tobjeto):
    ...
    def draw(self):
        img_key = "oikawa2.png" if self.sim.estado_saque == 2 else "oikawa1.png"
        
        x_px = int(0.7 * self.sim.PX_M) 
        y_px = int(self.sim.suelo - (2 * self.sim.PX_M))
        
        if self.sim.estado_saque == 2:
...
class TVoley(Tsim):
    def __init__(self, **kwargs):
        ...
        # Posición inicial de la pelota (en metros)
        self.pos_inicio_balon = (1.5, 0.6) 
        
        # 0: Jugador sosteniendo la pelota (inicio)
        # 1: Pelota lanzada al aire
        # 2: Pelota rematada
        self.estado_saque = 0 
        ...
        self.ha_tocado_suelo = False
        self.distancia_impacto = None
        self.velocidad_y_anterior = 0 
        
        # Parámetros del tiro inicial hacia arriba antes del saque
        self.v_toss = 8.2      # Fuerza hacia arriba
        self.w_toss = 15.0    # Ligero giro hacia adelante inicial
        
        # Parámetros del remate (al sacar)
        self.v_lanzamiento = 21.0
        self.ang_lanzamiento = 6.0
        self.w_lanzamiento = 60.0
        
        # Límites visuales de altura para el remate (en metros)
        self.h_max_rojo_sup = 3.9
        self.h_verde_sup = 3.3
        self.h_verde_inf = 2.8
        self.h_min_rojo_inf = 1.5
        ...
        self.add_evento_tecla(pygame.K_q, self.toggle_guia)

    def ejecutar_saque(self):
        if self.estado_saque == 0:
            self.balon.lanzar(self.v_toss, 85, self.w_toss)
            self.estado_saque = 1
            self.texto_feedback = ""
            
        elif self.estado_saque == 1:
            y_actual = self.balon.posicion.y
            
            # Evaluar zona
            if self.h_verde_inf <= y_actual <= self.h_verde_sup:
                multiplicador = 1.0
                self.texto_feedback = "¡PERFECTO! (Rotación Añadida)"
            else:
                multiplicador = 0.2
                self.texto_feedback = "¡MAL TIMING! (Poca Rotación Extra)"
                
            self.timer_feedback = 120 
            
            # Sumar rotación del saque a la rotación actual de la pelota
            rotacion_actual = self.balon.body.angular_velocity
            rotacion_nueva = rotacion_actual + (self.w_lanzamiento * multiplicador)
            
            self.balon.lanzar(self.v_lanzamiento, self.ang_lanzamiento, rotacion_nueva)
            self.estado_saque = 2

    def toggle_magnus(self):
        self.magnus_activo = not self.magnus_activo

    def toggle_guia(self):
        self.mostrar_guia = not self.mostrar_guia
    ...
    def draw(self):
        ...
        # Dibujar guía visual
        if self.mostrar_guia:
            x_guia = int(2.3 * self.PX_M)
            w_guia = 12
            
            y_r_sup_ini = self.suelo - int(self.h_max_rojo_sup * self.PX_M)
            y_r_sup_fin = self.suelo - int(self.h_verde_sup * self.PX_M)
            
            y_v_ini = self.suelo - int(self.h_verde_sup * self.PX_M)
            y_v_fin = self.suelo - int(self.h_verde_inf * self.PX_M)
            
            y_r_inf_ini = self.suelo - int(self.h_verde_inf * self.PX_M)
            y_r_inf_fin = self.suelo - int(self.h_min_rojo_inf * self.PX_M)
            
            # Dibujar Zonas
            pygame.draw.rect(self.screen, (200,50,50), (x_guia, y_r_sup_ini, w_guia, y_r_sup_fin - y_r_sup_ini))
            pygame.draw.rect(self.screen, (50,200,50), (x_guia, y_v_ini, w_guia, y_v_fin - y_v_ini))
            pygame.draw.rect(self.screen, (200,50,50), (x_guia, y_r_inf_ini, w_guia, y_r_inf_fin - y_r_inf_ini))
            
            # Dibujar puntero con la altura actual de la pelota si está en el aire (antes del remate)
            if self.estado_saque == 1:
                y_balon = self.suelo - int(self.balon.posicion.y * self.PX_M)
        ...
        txt_vel = self.font.render(f"Velocidad: {vel_m:.1f} m/s", True, (30, 30, 30))
        txt_rpm = self.font.render(f"Topspin: {rpm:.0f} RPM", True, (30, 30, 30))
        color_mag = (0, 150, 0) if self.magnus_activo else (200, 0, 0)
        txt_mag = self.font.render(f"Magnus: {'ON' if self.magnus_activo else 'OFF'}", True, color_mag)
        
        txt_info1 = self.font.render("ESPACIO = Lanzar / Rematar | ESC = Reiniciar", True, (100, 100, 100))
        txt_info2 = self.font.render("M = Quitar Magnus | Q = Ocultar Guía", True, (100, 100, 100))
        ...
        self.screen.blit(txt_info1, (20, 20))
        self.screen.blit(txt_info2, (20, 50))
        
        # Feedback visual del timing
        if self.timer_feedback > 0:
            color_feed = (50, 200, 50) if "PERFECTO" in self.texto_feedback else (200, 50, 50)
            txt_feed = self.font.render(self.texto_feedback, True, color_feed)
            self.screen.blit(txt_feed, (self.width // 2 - 100, 100))
            self.timer_feedback -= 1
        
        if self.distancia_impacto is not None:
            txt_dist = self.font.render(f"Cayó a: {self.distancia_impacto:.2f} m", True, (200, 50, 50))
            self.screen.blit(txt_dist, (self.width - 250, 110))
            x_px = int(self.distancia_impacto * self.PX_M)
            pygame.draw.circle(self.screen, (220, 30, 30), (x_px, self.suelo), 6)
            pygame.draw.circle(self.screen, (255, 255, 255), (x_px, self.suelo), 3)
    ...
    def revisar_impacto(self):
        if self.estado_saque == 2 and not self.ha_tocado_suelo:
            vel_y_actual = self.balon.velocidad.y
            
            if self.velocidad_y_anterior < -0.1 and vel_y_actual >= 0 and self.balon.posicion.y < 0.5:
                self.ha_tocado_suelo = True
                self.distancia_impacto = self.balon.posicion.x
                
            self.velocidad_y_anterior = vel_y_actual
...
if __name__ == "__main__":
    ...
    FPS = 60
    substeps = 10
    velocidad_simulacion = 0.9  
    dt = (1.0 / FPS / substeps) * velocidad_simulacion
